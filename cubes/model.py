...
from .common import SimplifyingDict, get_logger, to_label
from .errors import *
...
def _json_from_url(url):
	...
	#print ('estou no _json_from_url',url)
	...
	* cube_*.json - files with cube definition, one cube per file
	...
	if isinstance(resource, str):
		...
		if parts.scheme in ('', 'file') and os.path.isdir(resource):
			#o modelo é uma directoria
			model = load_model_bundle(resource)
		else:
			#o modelo é um ficheiro ou url
...
def create_model(model, cubes=None, dimensions=None, translations=None):
	...
	if "dimensions" in model_desc:
		...
		all_dimensions = []
	print ('as all_dimensions sao ',all_dimensions, 'e as dimensions sao ', dimensions)

	if all_dimensions and dimensions:
		print ('tenho all_dimensions e dimensions')
		all_dimensions = all_dimensions[:] + dimensions
	elif not all_dimensions:
		print ('não tenho all_dimensions')
	...
	for desc in all_dimensions:
		...
		print ('o type de dimension e: ', type(dimension), dimension.name)
		model_dimensions[dimension.name] = dimension
	print ('model_dimensions ',model_dimensions)
	...
	if translations:
		for lang, trans in translations.items():
			model._add_translation(lang, trans)
	print ('model do create_model', model)
	return model
...
def create_dimension(obj, dimensions=None):
	...
	* If no levels are specified, then one level is created, with name
	...
	print ('Dimension do create dimension', d)
	return d
...
class Model(object):
	...
	def add_cube(self, cube):
		...
		print ('self.dimensions no modelo ', type(self.dimensions), self.dimensions)
...
#		return not self.__eq__(other)
	...
	def __getitem__(self, item):
		print ('my self_values e: ', self.levels,type(self.levels), item, type(item))
