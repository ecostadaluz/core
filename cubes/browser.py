...
def levels_from_drilldown(cell, drilldown):
	...
	* list of tuples in form (`dimension`, `hierarchy`, `level`).
	...
	print('este e o drilldown', drilldown)
	...
	for obj in drilldown:
		...
		print ('estes sao o dim hier e level ', dim, hier, level)#item, default, none
		...
		if level:
			...
			if str(cut_hierarchy) != str(hier):# mudado por ama cut_hierarchy != hier:
				#print (cut_hierarchy, hier, type(cut_hierarchy), type(hier))
...
#									  "%s" % (level, hier, dim))
			print('estes são o hier e o depth ', hier, depth)#default, 0
