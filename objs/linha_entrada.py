...
__model_name__ = 'linha_entrada.LinhaEntrada'
...
class LinhaEntrada(Model, View):
    def __init__(self, **kargs):
        Model.__init__(self, **kargs)
        self.__name__ = 'linha_entrada'
        self.__title__ = 'Linhas de Talão de Entrada'
        self.__model_name__ = __model_name__
        self.__list_edit_mode__ = 'inline'
        self.__get_options__ = ['produto']

        self.entrada = parent_field(view_order=1, name ='Entrada', args='style:visibility="hidden"', model_name='entrada.Entrada', nolabel=True, onlist=False, column='numero')
        self.ean = string_field(view_order=2, name ='EAN', size=45, onchange='ean_onchange')
        self.produto = choice_field(view_order=3, name ='Produto', args='required tabIndex="-1"', size=60, onchange='produto_onchange', model='produto', column='nome', options="model.get_opts('Produto', '_buyable()')")
        ...
        self.valor_total = currency_field(view_order=8, name ='Valor Total', args='readonly="readonly" tabIndex="-1"', size=20, sum=True, default=to_decimal(1))
