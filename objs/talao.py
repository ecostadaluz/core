...
class Talao(Model, View):
    def __init__(self, **kargs):
        Model.__init__(self, **kargs)
        self.__name__ = 'talao'
        self.__title__ = 'Talão de Venda'
        self.__model_name__ = __model_name__
        self.__list_edit_mode__ = 'edit'
        self.__order_by__ = 'talao.data, talao.id'
        self.__workflow__ = (
        ...
        self.__tabs__ = [
            ('Linhas de Talão de Venda', ['linha_talao']),
        ...
        self.cliente = choice_field(view_order=3, name ='Cliente', args='required', size=40, default='get_default_cliente()', model='terceiro', column='nome', options='model.get_terceiros()')
        ...
        self.vendedor = parent_field(view_order=7, name ='Vendedor', size=40, default="self.session.get('user', None)", model_name='users.Users', onlist=False, column='nome')
        ...
        self.active = boolean_field(view_order=12, name ='active?', args='hidden', default=False, nolabel=True, onlist=False, search=False)
        self.pagamentos = list_field(view_order=13, name ='Pagamentos', condition="documento='talao' and num_doc='{numero}'", model_name='linha_caixa.LinhaCaixa', list_edit_mode='edit', onlist = False)
        self.movs_contab = list_field(view_order=14, name ='Movimentos Contab.', condition="documento='talao' and num_doc='{numero}'", model_name='movimento.Movimento', list_edit_mode='edit', onlist = False)
        self.movs_stock = list_field(view_order=15, name ='Movimentos Stock', condition="documento='talao' and num_doc='{numero}'", model_name='stock.Stock', list_edit_mode='edit', onlist = False)
        self.linha_talao = list_field(view_order=16, name ='Linhas de Talão', condition="talao='{id}'", model_name='linha_talao.LinhaTalao', list_edit_mode='inline', onlist = False)
...
#       return options
    ...
    def record_lines(self, key):
        ...
        def get_results():
            try:
                ...
                from linha_talao import LinhaTalao
            record_lines = LinhaFacturaCliente(where="talao = '{talao}'".format(talao=key)).get()
            return record_lines
    ...
    def Imprime(self, key, window_id):
        ...
        record_lines = run_sql("SELECT linha_talao.*, produto.nome as nome_produto FROM linha_talao JOIN produto on produto.id = linha_talao.produto WHERE talao = '{talao}'".format(talao=record['id']))
        ...
        Line = '----------------------------------------' + '\n'
        VatLine = 'Iva Incluido a Taxa de 15%' + '\n'
    ...
    def Confirmar(self, key, window_id):
        ...
        movimento = Movimento(data=record['data'], numero=base_models.Sequence().get_sequence('movimento'), num_doc=record['numero'], descricao='Nosso Talão de Venda', diario=diario, documento='talao', periodo=periodo, estado='Confirmado', user=session.get('user', None), active=False).put()
        from stock import Stock
        stock = Stock(data=record['data'], numero=base_models.Sequence().get_sequence('stock'), num_doc=record['numero'], descricao='Nosso Talão de Venda', documento='talao', periodo=periodo, estado='Confirmado', user=session.get('user', None)).put()
        Talao(**record).put()
        from linha_talao import LinhaTalao
        record_lines = LinhaTalao(where="talao = '{talao}'".format(talao=record['id'])).get()
        if record_lines:
            ...
            for line in record_lines:
                ...
                LinhaMovimento(movimento=movimento, descricao=descricao, conta=conta_gastos, quant_debito=quantidade, debito=line['valor_total'], quant_credito=to_decimal('0'), credito=to_decimal('0'), user=session.get('user', None)).put()
                LinhaMovimento(movimento=movimento, descricao=descricao, conta=conta_mercadorias, quant_debito=to_decimal('0'), debito=to_decimal('0'), quant_credito=quantidade, credito=line['valor_total'], user=session.get('user', None)).put()
                LinhaStock(stock=stock, descricao=descricao, produto=line['produto'], armazem=armazem_vendas, quant_saida=quantidade, quant_entrada=to_decimal('0'), user=session.get('user', None)).put()
                LinhaStock(stock=stock, descricao=descricao, produto=line['produto'], armazem=armazem_cliente, quant_saida=to_decimal('0'), quant_entrada=quantidade, user=session.get('user', None)).put()
            return form_edit(key = key, window_id = window_id)
        else:
    ...
    def Facturar(self, key, window_id):
        ...
        factura = FacturaCliente(data=record['data'], notas=record['notas'], talao=record['id'], cliente=record['cliente'], residual=str(to_decimal('0')), estado='Rascunho', user=session.get('user', None)).put()
        record['factura'] = factura
        Talao(**record).put()
        #Muda o movimento de não active para active
        from movimento import Movimento
        movimentos = Movimento(where="documento='talao'and num_doc={num_doc} ".format(num_doc=record['id'])).get()
        if movimentos:
        ...
        if record_lines:
            ...
            for line in record_lines:
                quantidade = to_decimal(str(line['quantidade']))
                LinhaFacturaCliente(factura_cli=factura, produto=line['produto'], quantidade=quantidade, valor_total=line['valor_total'], desconto=line['desconto'], iva=line['iva'], valor_unitario=line['valor_unitario'], ean=line['ean'], user=session.get('user', None)).put()
        return form_edit(key = key, window_id = window_id)
    ...
    def Cancelar(self, key, window_id):
        ...
        from movimento import Movimento
        from linha_movimento import LinhaMovimento
        movimentos = Movimento(where="documento='talao' and num_doc={num_doc} ".format(num_doc=record['numero'])).get()
        if movimentos:
            for movimento in movimentos:
                new_movimento = {}
                new_movimento['user'] = session.get('user', None)
                ...
                for linhamovimento in linhas_movimento:
                    ...
                    new_linha_movimento['user'] = session.get('user', None)
                    LinhaMovimento(**new_linha_movimento).put()
        from stock import Stock
        from linha_stock import LinhaStock
        stocks = Stock(where="documento='talao' and num_doc={num_doc} ".format(num_doc=record['numero'])).get()
        if stocks:
            for stock in stocks:
                new_stock = {}
                new_stock['user'] = session.get('user', None)
                ...
                for linhastock in linhas_stock:
                    ...
                    new_linha_stock['user'] = session.get('user', None)
                    LinhaStock(**new_linha_stock).put()
        from linha_caixa import LinhaCaixa
        linhascaixa = LinhaCaixa(where="documento='talao'and num_doc={num_doc} ".format(num_doc=record['numero'])).get()
        if linhascaixa:
            for linhacaixa in linhascaixa:
                ...
                new_linha_caixa['user'] = session.get('user', None)
        ...
        if facturas_cli:
            for factura_cli in facturas_cli:
                ...
                factura_cli['user'] = session.get('user', None)
