...
__model_name__='leitura_tecnica.LeituraTecnica'
...
class LeituraTecnica(Model, View):
    def __init__(self, **kargs):
        Model.__init__(self, **kargs)
        self.__name__ = 'leitura_tecnica'
        self.__title__= 'Folhas de Leitura Tecnica'
        self.__model_name__ = __model_name__
        self.__list_edit_mode__ = 'edit'
        self.__order_by__ = 'int8(leitura_tecnica.numero) DESC'
        self.__workflow__ = (
        ...
        self.linha_leitura_tecnica = list_field(view_order=6, name ='Linhas de Leitura Técnica', condition="leitura_tecnica='{id}'", model_name='linha_leitura_tecnica.LinhaLeituraTecnica', list_edit_mode='inline', onlist = False)
    ...
    def Imprimir(self, key, window_id):
        ...
        record_lines = LinhaLeituraTecnica(where="leitura_tecnica = '{factura}'".format(factura=record['id'])).get()
        final_record_lines = []
        for line in record_lines:
            final_line = {}
            for field in LinhaLeituraTecnica().__fields__:
                final_line[field[0]] = get_field_value(line, field, LinhaLeituraTecnica())['field_value']
    ...
    def Confirmar(self, key, window_id):
        ...
        record['numero'] = base_models.Sequence().get_sequence('leitura_tecnica')
        ...
        movimento = Movimento(data=record['data'], numero=base_models.Sequence().get_sequence('movimento'), num_doc=record['numero'], descricao='Nossa Factura', diario=diario, documento='leitura_tecnica', periodo=periodo, estado='Confirmado', user=session['user']).put()
    ...
    def Cancelar(self, key, window_id):
        ...
        from movimento import Movimento
        movimento = Movimento(data=datetime.date.today(), numero=base_models.Sequence().get_sequence('movimento'), num_doc=record['numero'], descricao='Anulação de Nossa Factura', documento='leitura_tecnica', diario=diario, periodo=periodo, estado='Confirmado', user=session['user']).put()
        record['movimento'] = movimento
        LeituraTecnica(**record).put()
        from linha_leitura_tecnica import LinhaLeituraTecnica
        record_lines = LinhaLeituraTecnica(where="leitura_tecnica = '{factura}'".format(factura=record['id'])).get()
